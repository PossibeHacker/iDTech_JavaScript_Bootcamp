...
function Run() {
  ...
  focus = (focus + 1) % 2; // "%" is modulus, 
  // divide and return the remainder
  direction = checkPageBounds(direction, imgWidth, pos, pageWidth);
  // [24: switched from null to pageWidth, as instructed]
  img.src = pacArray[direction][focus];
  if (direction) { // if (direction) checks if the booleen or binary for True/1
...
setInterval(Run, 200) // this is my code

// This function determines the direction of PacMan based on screen edge detection. 
function checkPageBounds(direction, imgWidth, pos, pageWidth) {
  //TODO: Complete this to reverse direction upon hitting screen edge
  // can be "pos >= pageWidth - imgWidth" "pos + imgWidth >= pageWidth"
  /* pos is the top left, so "pos + imgWidth >= pageWidth" makes more sense 
